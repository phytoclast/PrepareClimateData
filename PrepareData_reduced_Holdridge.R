...
for (i in 0:11){
  Biomeclimate[,which(colnames(Biomeclimate)=='e01')+i] <- 58.93/365.25*
   Biomeclimate[,which(colnames(Biomeclimate)=='t01')+i]*(Biomeclimate[,which(colnames(Biomeclimate)=='t01')+i]>0)*Biomeclimate[,which(colnames(Biomeclimate)=='Dn01')+i] + 0.001
...
Biomeclimate <- 
  Biomeclimate[,c("ECO_ID","ECO_NAME","BIOME","biomname","Norm",
                  ...
                  "Deficit","Surplus")]
saveRDS(Biomeclimate, file='data/hBiomeclimate.RDS')
