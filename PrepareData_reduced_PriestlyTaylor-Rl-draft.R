...
#Rl = -1*(1.1 * Ra + -0.1) * (-0.02 + 0.261 * exp(-7.8e-4 * Tavg^2)) * 4.903e-9 *((Tmax + 273.1)^4 + (Tmin + 273.1)^4)/2
...
for (i in 0:11){
  ...
  Biomeclimate$Ra = 117.5 * (Biomeclimate$hs*sin(Biomeclimate$Latitude/360*2*3.141592)*sin(DaysMonth[i+1,]$declination) +
               cos(Biomeclimate$Latitude/360*2*3.141592)*cos(DaysMonth[i+1,]$declination)*sin(Biomeclimate$hs)) / 3.141592
  
  Biomeclimate$Rl = -1*(1.1 * Biomeclimate$Ra*0.75 + -0.1) * (-0.02 + 0.261 * exp(-7.8e-4 * (Biomeclimate[,which(colnames(Biomeclimate)=='t01')+i])^2)) * 4.903e-9 *((Biomeclimate[,which(colnames(Biomeclimate)=='th01')+i]
                                                                                                 + 273.1)^4 + (Biomeclimate[,which(colnames(Biomeclimate)=='tl01')+i]
                                                                                                               + 273.1)^4)/2
  
  Biomeclimate[,which(colnames(Biomeclimate)=='Dl01')+i] = Biomeclimate$Ra+Biomeclimate$Rl
...
Biomeclimate <- subset(Biomeclimate, 
                       select = -c(Ra, Rl, hs, Dn01, Dn02, Dn03, Dn04, Dn05, Dn06, Dn07, Dn08, Dn09, Dn10, Dn11, Dn12, 
